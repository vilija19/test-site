...
class HomeView implements \Vilija19\Core\Interfaces\RenderInterface
{
    ...
    public function render()
    {
        ...
        $view .= '          <button type="button"  @click="submit" id="delete-product-btn" class="btn btn-danger">MASS DELETE</button>'.PHP_EOL;
        $view .= '      </div>'.PHP_EOL;
        $view .= '  </div>'.PHP_EOL;
        $view .= '  <form id="products_form" method="POST" action="/delete" novalidate>'.PHP_EOL;
        ...
        foreach ($this->data['products'] as $product) {
            ...
            $view .= '              <input class="form-check-input delete-checkbox" name="selected-products[]" type="checkbox" value="'.$product->id.'">'.PHP_EOL;
